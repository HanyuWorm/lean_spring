...
def _to_time_from_ns(ns: int) -> time:
    # Python's 'time' only supports microsecond precision.
    # Convert ns to us.
    base = datetime(1970, 1, 1, tzinfo=timezone.utc) + timedelta(microseconds=ns // 1000)
    return base.time()
...
def struct_to_dict(value: Any, schema: Any, config: Optional[ConversionConfig] = None) -> Any:
    ...
    if schema_type == "STRUCT":
        # Handle Debezium logical type: VariableScaleDecimal
        if schema_name == "io.debezium.data.VariableScaleDecimal":
            try:
        ...
        # Handle Debezium logical type: ZonedTimestamp
        ...
        # Handle Kafka Connect / Debezium logical types based on integers
        if schema_name:
            try:
                if schema_name in ("org.apache.kafka.connect.data.Date", "io.debezium.time.Date"):
                    return date(1970, 1, 1) + timedelta(days=int(value))
                if schema_name in ("org.apache.kafka.connect.data.Time", "io.debezium.time.Time"):
                ...
                if schema_name == "io.debezium.time.NanoTime":
                    return _to_time_from_ns(int(value))
        ...
        # Native or NumPy numeric conversion
        if config.numeric_mode == "numpy" and _np is not None:
            try:
        ...
        # Handle Kafka Connect logical type: Decimal
        if schema_name == "org.apache.kafka.connect.data.Decimal":
            try:
                if hasattr(value, "toPlainString"):
                    return Decimal(str(value.toPlainString()))
                return Decimal(str(value))
